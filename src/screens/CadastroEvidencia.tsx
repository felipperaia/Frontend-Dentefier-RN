...
import { createEvidencia } from '../services/evidencias';
...
const submit = async () => {
  try {
    ...
    const response = await fetch('https://backend-dentefier.onrender.com/api/evidencias', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || 'Erro ao criar evidência');
    }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{padding:16}}>
      ...
      >
        ...
        {casos.map(c=>(
          <Picker.Item key={c._id} label={`${c.numeroCaso} – ${c.titulo}`} value={c._id}/>
        ))}
      </Picker>

      <Text style={styles.label}>Tipo</Text>
      <TextInput style={styles.input} value={form.tipo} onChangeText={t=>setForm(f=>({...f,tipo:t}))}/>
      ...
      >
        {usuarios.map(u=>(
          <Picker.Item key={u._id} label={`${u.username} (${u.role})`} value={u.username}/>
        ))}
      </Picker>
